function updateDockerTerraformVersion(async, chromium, fs, path, gitCommit) {
  async function getLatestTerraformVersion(callback) {
    ...
    const selector =
      "#main > div.page-header_root__FAAbV > div.version-context-switcher_root__DmKhm > select";
  ...
  function findTerraformVersionAndUrl(content) {
    const terraformRegex =
      /wget -q -O terraform\.zip\s+(https:\/\/releases\.hashicorp\.com\/terraform\/(\S+)\/terraform_\2_linux_amd64\.zip)/;
  ...
  function updateDockerFileWithNewTerraformVersion(
    options,
    latestVersion,
    callback
  ) {
  ...
  function updateTerraformVersionInShellScript(
    options,
    latestVersion,
    currentVersion,
    callback
  ) {
    const SHELL_SCRIPT_PATH = path.join(
    ...
    const updatedDockerfileContent = shellScriptContent.replace(
      currentTerraformVersionRegex,
      latestVersion
    );
  ...
  return function (options, callback) {
    ...
    const TASKS = [
      ...
      (latestVersion, continuation) =>
        updateDockerFileWithNewTerraformVersion(
          options,
          latestVersion,
          continuation
        ),
      (latestVersion, currentVersion, continuation) =>
        updateTerraformVersionInShellScript(
          options,
          latestVersion,
          currentVersion,
          continuation
