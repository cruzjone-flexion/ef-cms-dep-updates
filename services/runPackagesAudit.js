function runPackagesAudit(async, execute) {
  ...
  const CAVEAT_PACKAGES = ["quill", "artillery", "axios", "posthog-node"];
  ...
  function stopApplicationIfThereAreVulnerabilities(auditReport, callback) {
    ...
    if (vulnerabilities.length)
      throw new Error(
        `There are vulnerabilities on the following packages => ${vulnerabilities.join(
          ", "
        )}`
      );
    callback();
  ...
  return function (options, callback) {
    const TASKS = [
      ...
      (auditReport, continuation) =>
        stopApplicationIfThereAreVulnerabilities(auditReport, continuation),
    ];
