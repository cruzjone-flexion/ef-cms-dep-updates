function updateDockerAwsCliVersion(
  ...
  async function getLatestAwsCliVersion(callback) {
    ...
    const selector =
      "#repo-content-pjax-container > div > div > div > div.Box > div.Box-body.p-0 > div > div > div > div.d-flex > div.d-flex.inline.pr-2.pb-1.col-12 > h2 > a";
  ...
  function findAwsCliVersionAndUrl(content) {
    const awsCliRegex =
      /curl "(https:\/\/awscli\.amazonaws\.com\/awscli-exe-linux-x86_64-(\d+\.\d+\.\d+)\.zip)"/;
  ...
  function updateDockerFileWithNewAwsCliVersion(options, newVersion, callback) {
    ...
    const updatedAwsCliUrl = awsCliInfo.url.replace(
      awsCliVersionRegex,
      newVersion
    );
  ...
  return function (options, callback) {
    ...
    const TASKS = [
      ...
      (awsCliLatestVersion, continuation) =>
        updateDockerFileWithNewAwsCliVersion(
          options,
          awsCliLatestVersion,
          continuation
        ),
