function updateCypressBaseImage(
  ...
  function findHighestVersions(availableVersions) {
    ...
    for (const currentIteration of availableVersions) {
      ...
      if (
        compareVersions(currentIteration.chrome, highestVersions.chrome) === -1
      )
        continue;
      if (
        compareVersions(currentIteration.chrome, highestVersions.chrome) === 1
  ...
  async function getLatestCypressBaseVersion(callback) {
    const url =
      "https://hub.docker.com/r/cypress/browsers/tags?page=1&ordering=last_updated&name=node-18.";
    ...
    await page.goto(url);
    const selector =
      "#mainContainer > div > div > div.MuiStack-root.css-1821gv5 > div.MuiContainer-root.MuiContainer-maxWidthLg.css-2jualf > div > div.styles-module__container___Aft3y > div.styles-module__list___Pgwbr > div > div > div.styles-module__header___orFbe > div > div.styles-module__title___emfjy > div.styles-module__title_name___dNJgN > a";
  ...
  function findCypressBaseVersion(content) {
    ...
    if (!match)
      throw new Error("Unable to find the Cypress Base info in Dockerfile");
    const result = match[1].trim();
    return result;
  }

  function updateDockerFileWithNewCypressBaseVersion(
    options,
    cypressBaseLatestVersion,
    callback
  ) {
  ...
  return function (options, callback) {
    const commitMessage =
      "DepUpdate: Updated Cypress/Base image version in Dockerfile";
    const TASKS = [
      (continuation) => getLatestCypressBaseVersion(continuation),
      (cypressBaseLatestVersion, continuation) =>
        updateDockerFileWithNewCypressBaseVersion(
          options,
          cypressBaseLatestVersion,
          continuation
        ),
    ...
    async.waterfall(TASKS, () => {
      console.log(
        "Completed Update of Cypress/Base image version in Dockerfile"
      );
      callback();
    });
  };
