function updateAWSProviderForTerraform(async, chromium, gitCommit, fs, path) {
  async function getLatestVesrionOfAWSProviderForTerraform(callback) {
    ...
    const selector =
      ".provider-overview-metadata-item > .provider-overview-metadata-content";
    await page.waitForSelector(selector);
    const latestVersion = await page
      .$eval(selector, (element) => element.innerText)
      .catch(() => null);
    if (!latestVersion)
      throw new Error("Unable to find latest version of AWS provider");
    ...
    function traverseDirectory(currentPath) {
      ...
      files.forEach((file) => {
        ...
        if (stats.isDirectory()) {
          ...
          if (
            fileContent.includes("required_providers") &&
            fileContent.includes("aws = ")
          ) {
  ...
  function updateTerraformFileWithLatestAwsProviderVersion(
    pathToFile,
    latestVersion,
    callback
  ) {
    ...
    if (!currentVersion) {
      console.log(
        `\tCould not find the current AWS Provider in this file (${pathToFile})`
      );
      return callback();
    }
  ...
  function updateAllTerraformFilesWithNewProviderVersion(
    options,
    latestVersion,
    callback
  ) {
    const TASKS = getAllTerraformFilesWithAwsProviderDefined(
      options.PATH_TO_REPO
    ).map(
      (pathToFile) => (continuation) =>
        updateTerraformFileWithLatestAwsProviderVersion(
          pathToFile,
          latestVersion,
          continuation
        )
  ...
  return function (options, callback) {
    ...
    const TASKS = [
      ...
      (latestVersion, continuation) =>
        updateAllTerraformFilesWithNewProviderVersion(
          options,
          latestVersion,
          continuation
        ),
